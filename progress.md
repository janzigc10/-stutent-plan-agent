...
## 当前阶段
Plan 1 至 Plan 7 已全部完成，项目当前进入真机自用内测阶段：先把产品放到手机里真实使用，验证核心链路是否跑通，并在日常使用中持续暴露问题。当前活跃计划为 Plan 8《真机内测与体验收尾》，Task 2《打通手机访问当前前后端与 WebSocket》已完成，Task 3《验收 PWA 安装、启动和更新链路》已启动，当前阻塞在“真机 PWA 安装需要 HTTPS origin”。
...
## 下一 Session 第一件事
- 打开 `docs/superpowers/plans/2026-04-21-plan8-mobile-beta-stabilization.md`，从 Task 3 继续，先解决真机 PWA 需要 HTTPS origin 的访问路径。
- 在以下三种路线里选一个最省事的 HTTPS 方案，再用手机访问 `4173` 对应入口完成安装验收：
  - 临时公网 HTTPS 隧道，直接转发本地 preview/dev server
  - 同域部署的临时可访问 HTTPS 环境
  - 局域网可访问且已受信任的本地 HTTPS 证书方案
- 一旦 HTTPS 路径打通，优先在手机上重做 PWA 安装，并继续跑“登录 -> 聊天 -> 上传课表图片 -> 确认导入 -> 看日历”。

## 当前阻塞 / 风险
- 目前最大的前置条件不是功能本身，而是手机如何稳定访问当前前后端与 WebSocket。
- 真机 PWA 安装当前被 HTTPS 要求卡住：LAN 上的 HTTP preview 不是 secure context，`navigator.serviceWorker` 不可用。
